...
class GenSeerMarket(gl.Contract):
    ...
    def create_market(
        ...
        market = {"market_id": market_id, "creator": creator_address, "title": title,
    ...
    def stake(self, market_id: str, outcome_index: u256, amount: u256) -> str:
        ...
        existing_stakes.append({"user": user_address, "outcome_index": idx, "amount": amt,
    ...
    def settle_market(
        ...
        if not liquidity_valid:
            ...
            self._save_market(market_id, market)
            settlement = {"market_id": market_id, "verdict": "invalid",
...
- If evidence is absent, contradictory, or insufficient, return inconclusive."""
        def leader_fn():
            return gl.nondet.exec_prompt(settlement_prompt, response_format="json")
        ...
        try:
            ...
            raw_ruling = json.dumps(result) if isinstance(result, dict) else str(result)
        except Exception:
            raw_ruling = '{"verdict":"inconclusive","winning_outcome_index":null,"winning_outcome_label":null,"confidence":1,"evidence_strength":"insufficient","sentiment_analysis":"LLM unavailable","metrics_analysis":"LLM unavailable","sceptic_analysis":"LLM unavailable","context_analysis":"LLM unavailable","reasoning":"Settlement LLM call failed. Inconclusive verdict applied. Refund recommended.","evidence_summary":"Unable to process","counterarguments":"","refund_recommended":true}'
        ...
        if ruling["verdict"] == "settled" and ruling["winning_outcome_index"] is not None:
            ...
            market["settled_at"] = now
        self._save_market(market_id, market)
        settlement = {"market_id": market_id, "verdict": ruling["verdict"],
